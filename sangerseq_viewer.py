...
if uploaded_gbk:
    # Save the uploaded GenBank file to a temporary location
    temp_gbk_path = "./temp_genbank.gbk"  # Define a temporary path
    with open(temp_gbk_path, "wb") as f:
        f.write(uploaded_gbk.read())

if uploaded_abi:
    # Save the uploaded GenBank file to a temporary location
    temp_abi_path = "./temp_abi.ab1"  # Define a temporary path
    with open(temp_abi_path, "wb") as f:
        f.write(uploaded_abi.read())
...
predict = st.sidebar.button('View Chromatogram')

if predict:
    # Call your view_sanger function and generate the plot.png
    view_sanger(temp_gbk_path, temp_abi_path, output="plot.png", output_fasta="sequences.fasta")

    # Display the generated image
    st.image("plot.png")

    # Assuming you have the path to your text file
    text_file_path = "sequences.fasta"

    # Read the contents of the text file
    with open(text_file_path, "r") as file:
        text_content = file.read()

    # Display the text content in your Streamlit app
    st.text("Fasta sequences:")
    st.text(text_content)  # Display the text content

    st.text("⚠️ ATTENTION! Once the button is clicked the app restarts.")
    download_button = st.download_button(
    label="Download Image",
    data=open("plot.png", "rb").read(),
    key="download_button",  # Provide a unique key for this element
    file_name="plot.png",  # Specify the desired file name
    )

    if download_button:
        st.success("👉 Click the button above to download the image.")

if not predict:
    st.warning('👈 Enter files and click "view chromatogram"!')
